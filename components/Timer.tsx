...
const ZenVisualizer: React.FC<{ active: boolean }> = ({ active }) => (
  <div className="relative flex flex-col items-center mt-10 w-48 h-12 justify-center">
    ...
    <style>{`
      @keyframes zen-line { 0%, 100% { transform: scaleX(0.8); opacity: 0.3; } 50% { transform: scaleX(1.2); opacity: 0.7; } }
      @keyframes zen-ping { 0% { transform: scale(1); opacity: 0.8; } 100% { transform: scale(4); opacity: 0; } }
...
const Timer: React.FC = () => {
  const [state, setState] = useState<TimerState>(() => {
    ...
    return { mode: TimerMode.FOCUS, isActive: false, endTime: null, timeLeftWhenPaused: FOCUS_DURATION };
  });
  ...
  // 提供預設值 0 以符合 requestAnimationFrame(updateDisplay) 與 updateDisplay() 的雙重調用
  const updateDisplay = useCallback((_timestamp: number = 0) => {
    if (state.isActive && state.endTime) {
      ...
      if (diff <= 0) {
        ...
        saveState({ mode: nextMode, isActive: false, endTime: null, timeLeftWhenPaused: nextDuration });
    ...
    <div className="flex flex-col items-center w-full">
      ...
      <div className="flex flex-col items-center">
        <div className="flex gap-6 items-center">
          <button onClick={toggleTimer} className="w-20 h-20 flex items-center justify-center bg-white text-zen-matcha rounded-full shadow-xl hover:scale-105 active:scale-95 transition-all group">
            {state.isActive ? <Pause className="w-10 h-10 transition-transform group-hover:rotate-90" /> : <Play className="w-10 h-10 ml-1 transition-transform group-hover:scale-110" />}
          </button>
          <button onClick={resetTimer} className="w-16 h-16 flex items-center justify-center bg-white/10 text-white rounded-full border border-white/20 hover:bg-white/25 transition-all active:scale-90 group">
